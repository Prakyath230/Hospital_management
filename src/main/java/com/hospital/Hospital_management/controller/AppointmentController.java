...
public class AppointmentController {
    ...
    @GetMapping("/details")
    public ResponseEntity<AppointmentResponseDto> getMyAppointment(){
        return ResponseEntity.ok(appointmentService.getAppointmentDetails());
    }

    @PreAuthorize("hasRole('ADMIN')")
    @PutMapping("/approve/{id}")
    public ResponseEntity<ResponseDto> approveAppointment(@PathVariable Long id){
        return ResponseEntity.ok(appointmentService.approveAppointment(id));
    }

    @PreAuthorize("hasRole('ADMIN')")
    @PutMapping("/reject/{id}")
    public ResponseEntity<ResponseDto> rejectAppointment(@PathVariable Long id){
